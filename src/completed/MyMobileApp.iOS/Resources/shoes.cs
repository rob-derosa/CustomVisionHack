...
namespace MyMobileApp.iOS {
	...
	/// </summary>
	public class shoesInput : NSObject, IMLFeatureProvider
	{
		...
		public shoesInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class shoesOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public shoesOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class shoes : NSObject
	{
		readonly MLModel model;

		public shoes ()
		{
			var url = NSBundle.MainBundle.GetUrlForResource ("shoes", "mlmodelc");
			NSError err;
		...
		public static shoes Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new shoes (model);
		}

		/// <summary>
		/// Make a prediction using the standard interface
		/// </summary>
		/// <param name="input">an instance of shoesInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public shoesOutput GetPrediction (shoesInput input, out NSError error)
		{
			...
			return new shoesOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public shoesOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new shoesInput (data);
